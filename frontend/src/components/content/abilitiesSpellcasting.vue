<template>
    <div>
        <div class="defaultContainer">
            <div class="innerContainer accordion accordionSpell" v-for="(ability, index) in localCharacter.spells" :style="{'order': ability.levelRequirement}">
...
function removeAccordion(index) {
    localCharacter.value.spells.splice(index, 1);
    emit('update:characterData');
}

function addAccordion() {
    localCharacter.value.spells.push(
        {
        "name": "XXX",
        "useTime": "Action",
        "levelRequirement": "9",
...
function updateText(newContent, index){
    localCharacter.value.spells[index].description = newContent.value;
    emit('update:characterData');
}
</script>
